...
int http_read_line(int fd, char *buf,int t)
{
    // warnx("t value is %d\n",t);
   size_t i = 0;
    for (;;)
...
const char *http_request_line(int fd, char *reqpath, size_t reqpath_len, char *env, size_t *env_len)
{
    ...
    if (http_read_line(fd, buf,4096) < 0)
    ...
    *sp2 = '\0';
    ...
    envp += snprintf(envp,4096,"REQUEST_METHOD=%s", buf) + 1;
    envp += snprintf(envp,4096-strlen(envp),"SERVER_PROTOCOL=%s", sp2) + 1;

    /* parse out query string, e.g. "foo.py?user=bob" */
    if ((qp = strchr(sp1, '?')))
    {
        *qp = '\0';
        envp += snprintf(envp,4096-strlen(envp), "QUERY_STRING=%s", qp + 1) + 1;
    }

    /* decode URL escape sequences in the requested path into reqpath */
    url_decode(reqpath, sp1, reqpath_len);

    envp += snprintf(envp,4096-strlen(envp), "REQUEST_URI=%s", reqpath) + 1;
    envp += snprintf(envp,4096-strlen(envp), "SERVER_NAME=zoobar.org") + 1;
